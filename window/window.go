...
func (win *Window) setupWidgetWindow() {
	...
	// Размер и положение окна ведёт ОС: виджетный ресайз за края выключаем
	// явным признаком, а не через наличие OnDragMove (иначе приложение,
	// забравшее перемещение себе, теряло бы ресайз — см. ENGINE_ISSUES).
	ww.SetNativeHosted(true)
