...
// GetFlag возвращает флаг активной темы (def, если не задан).
//
// Флаги описывают выбор вида, а не величину: центрировать ли кнопки панели,
// показывать ли подпись у кнопки «Пуск». Компоненту такой вопрос задаётся
// как «что просит тема», а не «какая тема активна».
func (m *Manager) GetFlag(k Key, def bool) bool {
	m.mu.RLock()
	t := m.active
	m.mu.RUnlock()
	if t == nil {
