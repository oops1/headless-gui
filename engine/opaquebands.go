...
// по одной на строку.
//
// nil означает «выкраивать нечего»: картинки нет, в ней нет ни одной
// закрашенной точки — или прозрачное в ней только по краям.
//
// Последнее важно. Скруглённые углы меню прозрачны ВСЕГДА, и если считать их
// поводом для выкройки, потребитель будет перестраивать форму окна на каждый
// кадр — на каждое движение курсора по пунктам, — а окно от этого мигает на
// глазах. Выигрыш при этом нулевой: несколько точек в углах. Выкройка нужна
// только там, где дыра НАСТОЯЩАЯ — как между меню и его каскадным потомком.
...
// bandHoleThreshold — насколько строке позволено не дотягивать до полной
// ширины, прежде чем это считается дырой.
//
// Скруглённые углы съедают у крайних строк радиус слева и справа — это
// единицы и первые десятки точек. Настоящая дыра — место между меню и его
// каскадным потомком — измеряется сотнями. Порог посередине разделяет их
// уверенно и не требует знать радиус темы.
const bandHoleThreshold = 48

// hasRealHole сообщает, есть ли в картинке прозрачное, не сводящееся к
// скруглению краёв: строка без единой закрашенной точки, строка, разорванная
// на несколько отрезков, или строка, недобирающая до полной ширины больше
// порога.
func hasRealHole(img *image.RGBA) bool {
	b := img.Bounds()
	buf := make([]span, 0, 4)
	for y := b.Min.Y; y < b.Max.Y; y++ {
		buf = rowSpans(buf[:0], img, y)
		if len(buf) == 0 {
			return true // пустая строка — сквозная дыра
		}
		if len(buf) > 1 {
			return true // строка разорвана — между отрезками дыра
		}
		var covered int
		for _, sp := range buf {
			covered += sp.x1 - sp.x0
		}
		if b.Dx()-covered > bandHoleThreshold {
			return true
		}
	}
	return false
