...
func New(width, height, fps int) *Engine {
	...
	// Слой данных (биндинги/{Loc}/live-коллекции) и авто-damage виджетов
	// сообщают об изменениях UI — нужно для рендера по запросу (см.
	// invalidate.go). Регистрируемся в ШИРОКОВЕЩАТЕЛЬНОМ реестре приёмников:
	// несколько живых движков (главное окно + hosted-диалоги/попапы, а в
...
func (e *Engine) ShowModal(m widget.ModalWidget) {
	// Тема применяется ЗДЕСЬ, а не только в SetTheme.
	//
	// SetTheme красит дерево и уже показанные модалки, а диалог, созданный
	// после смены темы, оставался с цветами, которые виджеты получили при
	// создании. У большинства это незаметно — они читают палитру во время
	// отрисовки, — но виджет, кеширующий цвета у себя (DataGridWidget),
	// оставался с умолчаниями: в светлой теме таблица внутри диалога рисовалась
	// тёмной.
	//
	// Следствие, о котором надо знать: цвета, выставленные приложением на
	// виджетах диалога вручную ДО показа, будут переписаны темой. Ровно это
	// уже происходило с показанной модалкой при SetTheme, и разное поведение у
	// двух путей было бы хуже, чем предсказуемое одинаковое. Поддерево со
	// своей темой (ThemeScope) обходится стороной, как и в SetTheme.
	e.frameMu.Lock() // массовая мутация цветов — не во время отрисовки
	widget.ApplyThemeTree(m, widget.CurrentTheme())
	e.frameMu.Unlock()
