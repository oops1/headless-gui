...
**Сборка с AVX2 (по желанию).** При сборке с `GOEXPERIMENT=simd` на Go 1.27+ (amd64) самые горячие пиксельные циклы — смешивание маски для текста и сглаженных фигур, полупрозрачная заливка, перестановка каналов при выводе, размытие подложки — работают на AVX2: полный кадр дешевле примерно на 39 %, вывод кадра 1080p в окно быстрее в 2,9 раза, `BlurRGBA` — в 8,4 раза. Результат совпадает со скалярным бит в бит; векторный набор выбирается при запуске, только если процессор умеет AVX2 и набор прошёл самопроверку, иначе — или с `HEADLESS_GUI_NOSIMD=1` — работает скалярный путь. Подробности — [GUIDE.md](GUIDE.md), «Векторные ядра».
...
- **MergeView** — трёхстороннее слияние: наше, база и их сверху со строками, выровненными по блокам, и редактируемый итог снизу; решения по блоку (наше / их / база / оба) кнопкой, меню или Alt+1/2/3, нерешённый конфликт уходит в итог маркерами git (стиль merge или diff3)
- **DatePicker** — поле даты с выпадающим месячным календарём: дату можно набрать или выбрать, набор терпим к виду, формат и первый день недели берутся из таблиц строк языка интерфейса (встроены RU и EN, иначе ISO 8601), границы выбора
- **Что нужно редактору** — `win.SetTitle` и `win.SetOnCloseRequest` (вопрос «Сохранить изменения?» перед закрытием окна), виджет, забирающий Tab себе (`TabAcceptor`), сглаженные пути с дробными координатами и сопряжениями на изгибах (`PathShapes`: кубические кривые, ломаные, заливки), цвета сравнения и подсветки кода в теме, дочерние узлы доступности у виджета, рисующего содержимое сам
- **Пиксельные ядра на AVX2** — сборка с `GOEXPERIMENT=simd` по желанию (Go 1.27+, amd64): текст, сглаженные фигуры, заливки, размытие и вывод кадра на AVX2, бит в бит как скалярный путь, с проверкой процессора и самопроверкой при запуске — при отказе обход автоматический
...
| MergeView | `MergeView` | Трёхстороннее слияние: наше, база и их сверху, редактируемый итог снизу, решения по блокам, маркеры конфликта git |
| DatePicker | `DatePicker` | Поле даты с выпадающим месячным календарём: формат и первый день недели по языку, границы выбора |
...
webdemo/       Минимальный пример стриминга
    ...
    mergedemo/     Разрешение конфликта слияния на MergeView
    socialpreview/ Картинка превью репозитория (social_preview.png), рисует движок
...
Ядрам на AVX2 нужен **Go 1.27+** и сборка с `GOEXPERIMENT=simd` на amd64 (API экспериментального `simd/archsimd` между 1.26 и 1.27 менялся). Со старым тулчейном, без эксперимента или на другой архитектуре движок собирается как обычно и работает скалярным путём — `go.mod` остаётся на `go 1.22`.
