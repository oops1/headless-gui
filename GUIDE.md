...
целиком (по умолчанию пропуск включён).
...
Перетаскивание окна не меняет пиксели, а переносит их. `widget.NotifyMove(src, dst)`
объявляет перенос; в RDP ему соответствует пара команд через кэш поверхности
вместо сотни килобайт.
