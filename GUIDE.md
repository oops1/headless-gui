...
модалкой, тени) и перестановка каналов при выводе в окно ОС — умеют работать
...
Нужен Go 1.26 или новее и amd64; векторный путь выбирается при запуске, только
если процессор умеет AVX2. Во всех остальных случаях — сборка без эксперимента,
старый тулчейн, arm64, старый процессор — работают прежние скалярные циклы, и
для библиотеки ничего не меняется: `go.mod` остаётся на `go 1.22`.
