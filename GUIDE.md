...
**Свой кегль.** Есть у кнопки, флажка и списка; раньше кегль был только у
`Label`, `TextBox` и `TextInput`, а остальные жёстко брали
`DefaultFontSizePt` — единственным рычагом оставалось менять его на всё
приложение:

```go
btn.FontSize = 18   // пункты; 0 = общий размер
chk.FontSize = 9
dd.FontSize = 14
```

В XAML — атрибут `FontSize` у `<Button>`, `<ToggleButton>`, `<CheckBox>` и
`<ComboBox>`.

**Усечение подписи.** Не влезающая подпись кнопки заменяется на «начало…»:
клиппинга у кнопки нет, и раньше длинный текст вылезал за край и наезжал на
соседний виджет. Ширина считается по фактическому размеру кнопки за вычетом
иконки и отступов, поэтому короткая подпись на широкой кнопке не трогается.

### Измерение содержимого

`StackPanel`, `ScrollView` и `Expander` умеют сказать, сколько места им нужно:

```go
w, h := sp.DesiredSize()   // сумма детей + промежутки + отступы
sp.Relayout()              // пересчитать столбик после смены состава
sv.FitContent()            // высота прокрутки по детям
```

`DockPanel` и `WrapPanel` перемеряют ребёнка без явного размера на каждой
раскладке: кнопка или флажок со сменившейся подписью получают новую ширину, а
размер, заданный кодом, остаётся. О смене текста панель сама не узнаёт — после
`btn.SetText(…)` из кода позовите `dock.Relayout()`. Дерево из разметки при смене
языка переразмечается само.

Свёрнутый `Expander` просит высоту одного заголовка и в столбике не держит
место под спрятанное содержимое; скрытый ребёнок не занимает ни своего места,
ни промежутка после себя. Отсюда и типичный случай — сворачиваемая группа
внутри прокрутки:
...
Двухпроходного Measure/Arrange здесь нет: `DesiredSize` спрашивается
интерфейсом, и контейнер обращается к нему только у тех детей, кто его
реализует, — всем прочим по-прежнему достаются их собственные границы.

### Separator

```go
s := widget.NewSeparator()          // горизонтальный
v := widget.NewVerticalSeparator()
s.Thickness = 2                     // толщина, 0 = один пиксель
s.Margin = 20                       // отступ от краёв ВДОЛЬ линии
```

В XAML — `<Separator/>` с атрибутами `Orientation`, `Thickness`, `Margin`,
`Background`. Ориентация выводится из размеров: заданная ширина без высоты
означает горизонтальную линию.

Цвет берётся из темы (`Theme.Border`) и обновляется вместе с ней. Линия
рисуется по центру отведённой области: разделителю обычно отводят полосу в
несколько пикселей, и линия у верхнего края зрительно принадлежала бы
предыдущей группе, а не промежутку между группами.

Для пояснительных подписей под параметрами в теме есть `Theme.SecondaryText` —
отдельно от `Disabled`, который означает «этим нельзя пользоваться».
...
**Значок в начале поля.** Строка поиска с лупой, поле пароля с замком:

```go
inp.LeadingIcon = searchIcon // image.Image
inp.LeadingIconSize = 16     // 0 = по высоте поля
```

В XAML — атрибут `Icon` (и `IconSize`). Текст и каретка сдвигаются вправо на
ширину значка: класть значок отдельным виджетом поверх поля не нужно, и клик по
нему больше не уходит мимо поля.

**Секрет читается байтами.** `GetText()` отдаёт неизменяемую Go-строку: занулить
её нельзя ничем, она живёт в куче до сборки мусора и попадает в дампы памяти.
Приложению, у которого секрет не должен превращаться в `string`:

```go
secret := inp.TakeSecret() // []byte; поле очищается тем же вызовом
defer clear(secret)
vault.Unlock(secret)

inp.WipeSecret() // на отмене диалога: прочитать не понадобилось, стереть надо
```

`Take`, а не `Get`: пароль читают один раз, чтобы отдать в хранилище или в
транспорт, и разделять чтение и очистку значит оставить окно, в котором секрет
лежит в двух местах. Работает и на обычном поле — режим пароля скрывает символы
на экране, а к способу чтения отношения не имеет.
...
**Значок пункта.** Картинкой, а не путём: значки приложения обычно
вкомпилированы и растеризованы в цвет темы, файла на диске у них нет.

```go
menu.SetItems([]widget.MenuItem{
    {Text: "Копировать путь", Icon: iconCopy},              // image.Image
    {Text: "Открыть в терминале", Icon: iconTerm, IconSize: 14},
    {Text: "Показывать скрытые", Icon: iconEye, Checkable: true, Checked: true},
    {Text: "Без значка"}, // подпись встанет в тот же столбик
})
```

В разметке — `Icon="icons/copy.png"` и `IconSize="14"` у `<MenuItem>`; путь
ограничен каталогом XAML-файла или `fs.FS`, как у `Button.Icon`. Не прочитался
— пункт остаётся без значка: меню без картинки понятнее, чем окно, не
открывшееся из-за неё.

Зона под значки отводится ВСЕМУ меню, если значок есть хоть у одного пункта, —
то же правило, что у `Checkable`: иначе подписи пунктов без значка стояли бы
левее соседей. `IconSize` — сторона в точках, ноль означает «по высоте пункта»;
размер крупнее пункта ужимается, иначе значок разрезал бы соседние строки.
Отметка и значок не спорят за место: отметка слева, значок за ней, подпись за
значком. Подменю (в том числе у `MenuBar`) рисует значки тем же кодом.
...
item.Foreground // свой цвет текста (нулевая альфа = цвет темы)
item.Bold       // жирное начертание
...
переключении. Для этого колбэк:

```go
tw.Tree.ItemStyle = func(it *treeview.TreeViewItem) (color.RGBA, bool, bool) {
    if it.Tag == app.ActiveRepo() {
        return color.RGBA{}, true, true // жирным, цвет темы
    }
    return color.RGBA{}, false, false   // ok=false — «решай сам»
}
```

Возврат `ok=false` означает «решай сам»: тогда смотрятся поля `Foreground` и
`Bold` самого узла. Колбэк зовётся на каждый видимый узел в каждом кадре —
тяжёлой работы внутри быть не должно.
...
**Ширина колонки.** У колонки есть собственные `SetMinWidth`/`SetMaxWidth`, и
они участвуют и в распределении `*`-колонок, и в ресайзе мышью: в узком окне
колонка «Статус» не ужимается до неразличимой, а «Ресурс» отдаёт ей место.
Star-колонки делятся в несколько кругов — упёршаяся в свою границу колонка
отдаёт остаток тем, кто ещё может тянуться.

**Длинный текст.** Текстовая колонка обрезает значение многоточием по ширине
ячейки. Шаблонной колонке та же функция доступна:

```go
s := datagrid.EllipsizeText(cdc.DrawCtx, longText, maxW, cdc.FontSize)
```

**Фигуры в ячейке.** `cdc.DrawCtx` умеет `FillEllipseAA` и `FillRoundRect` —
цветной кружок статуса рисуется на месте, без заранее растеризованных картинок.

Линии и контуры — там же, весь остаток `AAShapes` движка:

```go
cdc.DrawCtx.DrawLineAA(x1, y1, x2, y2, 2, col)               // отрезок
cdc.DrawCtx.StrokePolylineAA(pts, 2, false, col)             // ломаная
cdc.DrawCtx.StrokeEllipseAA(cx, cy, r, r, 2, col)            // кольцо
cdc.DrawCtx.FillPolygonAA(pts, col)                          // многоугольник
```

Ради них запрос и подавали: колонка графа коммитов — это дорожки, переходы
между ними наклонной линией и кольцо на точке слияния. Кольцо двумя заливками
(цветом дорожки, затем фоном) не изобразить: на выделенной строке фон другой, и
середина кольца окажется не того цвета.

Толщина — дробная: на сглаженном холсте линия в полторы точки существует.
Округляет только откат для холста без сглаживания (печать, тесты) — там фигура
ложится ступеньками, но ложится.

**Пустое состояние.** Пустая таблица — обычное первое состояние списка ключей
или журнала:

```go
dg.Grid.EmptyStateText = "Ключей пока нет"
dg.Grid.EmptyStateRenderer = func(c datagrid.EmptyStateContext) { ... } // своя отрисовка
```

Заголовки при этом остаются: по ним видно, что за таблица перед человеком.
...
scope.Dispose()               // дерево больше не нужно
```

Дерево из разметки подписано на смену языка и на модель, пока его не
освободят. Окно или диалог, который загружается заново при каждом открытии,
освобождайте при закрытии: `scope.Dispose()` для дерева из
`LoadUIFromXAMLBindings`, `widget.ReleaseXAML(root)` — для загруженного любой
другой `LoadUIFromXAML*`. Иначе каждая смена языка переводит и перекладывает
все выброшенные деревья.
...
Заголовок окна, диалога и докируемой панели тоже следует смене языка:
...
`SetTitle` у `Dialog`, `Window` и `DockPane` можно звать и из кода. Панель при
этом перекладывает менеджер: ширина корешка вкладки считается по подписи, и без
перекладки новый заголовок не поместился бы в старую ширину.
...
**Изменяемый размер.** По умолчанию выключен — диалог с вопросом и двумя
кнопками тянуть незачем:

```go
dlg.SetResizable(true)
dlg.SetMinSize(500, 400)   // ноль по оси = умолчание 200×120
dlg.SetContent(panel)      // ЭТОТ виджет занимает область содержимого
dlg.Resize(800, 600)
```

Растягивается только виджет из `SetContent`; обычные дети сдвигаются вместе с
диалогом. Растягивать всех, как это делает `widget.Window`, здесь нельзя:
диалоги расставляют кнопки и подписи абсолютными координатами внутри себя, и
«каждый ребёнок занимает всю область» превратило бы `MessageBox` в стопку
виджетов друг поверх друга.

Включённая растяжимость доходит и до окна ОС, если диалог показан в собственном
окне: рамка с ресайзом, минимум из `MinSize` и пересчёт холста при изменении.

`ShowModal` применяет к диалогу ТЕКУЩУЮ тему. Иначе виджет, кеширующий цвета у
себя (`DataGridWidget`), оставался бы с умолчаниями: диалог, созданный после
`SetTheme`, рисовал бы тёмную таблицу в светлой теме. Обратная сторона: цвета,
выставленные на виджетах диалога вручную ДО показа, будут переписаны темой —
ровно это уже происходило с показанной модалкой при `SetTheme`. Поддерево со
своей темой (`ThemeScope`) не трогается.

**Право остановить закрытие.** `OnClosing` спрашивают ПЕРЕД закрытием по Escape
и по ✕; `false` останавливает закрытие, и диалог остаётся в стеке:
...
dlg.OnClosing = func() bool {
    ...
    eng.ShowModal(askSave()) // спросить и дождаться ответа
    return false             // пока не закрываемся
}
```

Раньше вето не было вовсе: движок звал `OnCancel` и СРАЗУ ЖЕ `CloseModal`, так
что диалог с несохранёнными изменениями выталкивался из стека, не дождавшись
ответа на свой же вопрос, — приложению приходилось замечать это по `IsModal()`
и показывать диалог заново, теряя кадр.

`CancelAction` при остановленном закрытии НЕ зовётся: «отмена» — состоявшееся
...
только под ним. Окно настроек, устроенное как вертикальные вкладки браузера
(панель навигации во всю левую сторону, заголовок внутри неё), собрать было
нельзя:

```go
dlg.SetChromeless(true)                    // ни полосы, ни подписи, ни ✕
dlg.SetContent(root)                       // содержимое от самого верха
dlg.AddDragArea(image.Rect(0, 0, 240, 48)) // шапка панели тащит окно
dlg.ClearDragAreas()
```

Координаты области — ОТНОСИТЕЛЬНО левого верхнего угла диалога, а не
абсолютные: диалог двигают, и абсолютные пришлось бы пересчитывать после
каждого перетаскивания, то есть после каждого использования этой самой области.

Кнопка ✕ убирается вместе с полосой: рисовать её поверх чужой шапки движок не
вправе, он не знает, что там у приложения. Крестик рисует приложение и зовёт
`RequestClose`; закрытие по Escape остаётся.

Объявленная область СИЛЬНЕЕ лежащих в ней виджетов. У штатной полосы правило
обратное — там ✕ забирает нажатие себе, — но шапку, нарисованную приложением,
целиком составляют его же виджеты, и проверка «под курсором чей-то ребёнок»
отменяла бы перетаскивание всегда. Вырезать из области свои кнопки — забота
того, кто её объявил: объявите две области по бокам от кнопки вместо одной
сплошной.

`AddDragArea` и `ClearDragAreas` есть и у `widget.Window`. Нужны они
borderless-окну (`WindowStyleNone`): полосы у него нет, а значит и тащить его
было не за что — `WantsCapture` отвечал «нет» безусловно.

**Свои контролы в штатной полосе.** Убирать полосу целиком нужно не всегда:
чаще подпись и ✕ пусть рисует движок, а приложению нужны своё поле поиска в
полосе и кнопка, сворачивающая боковую навигацию.

```go
dlg.SetTitleBarContent(searchBox) // виджет в свободной части полосы
dlg.SetNavButton(true)            // кнопка сворачивания слева от подписи
dlg.SetNavIcons(iconMenu, nil)    // своя иконка (nil — встроенная «≡»)
dlg.OnNavToggle = func(collapsed bool) {
    nav.SetVisible(!collapsed)
    relayout()
}
dlg.SetContentPadding(0) // содержимое доходит до краёв окна
```

Геометрию начинки считает движок: после подписи (и кнопки сворачивания, если
она есть) и до ✕ — `TitleBarContentBounds()` отдаёт этот прямоугольник. Полоса
принадлежит движку, её высота зависит от темы, а справа в ней стоят его же
органы управления; приложению, которому нужна полоса целиком, по-прежнему
служит `SetChromeless`.

Нажатие на этот виджет НЕ тащит окно — иначе поле поиска нельзя было бы ни
выделить, ни прокрутить. Свободная часть полосы рядом с ним тащит по-прежнему.

Правее начинки движок оставляет свободную полоску в 72 точки — это ручка
перетаскивания. Поле во всю ширину не оставило бы в полосе ни одной точки, за
которую окно можно схватить мышью, а полоса заголовка — это в первую очередь
место, за которое окно двигают.

**Три штатные кнопки.** Диалог рисовал в полосе одну ✕: диалогу-вопросу больше
и не нужно. Диалогу, показанному в собственном окне ОС и растянутому на
пол-экрана, нужны все три:

```go
dlg.SetWindowButtons(true) // ─ □ ✕ вместо одной ✕
dlg.SetMaximized(true)     // глиф «развернуть» → «восстановить»
```

Сворачивает и разворачивает окно ОС, а не виджет: `OnMinimize` и
`OnMaximizeRestore` ставит нативный хост модалки при показе диалога в своём
окне. В headless (диалог нарисован внутри главного окна) сворачивать нечего —
там кнопки останутся без действия, и включать их незачем. Кнопки занимают место
в полосе: ✕ переезжает левее, начинка приложения ужимается.

Сворачивание делает приложение: движок не знает, что у него слева — панель
навигации, дерево или `SplitPanel`. Кнопка лишь держит состояние
(`IsNavCollapsed`, `SetNavCollapsed`) и зовёт `OnNavToggle`.

То же самое есть у `widget.Window`: `SetTitleBarContent`, `SetNavButton`,
...
виджет скрыт, кнопка сворачивания не показывается, а подпись рисуется как
обычно.

**Отступ содержимого.** У диалога он был константой — 14 точек по горизонтали
и 12 по вертикали:

```go
dlg.SetContentPadding(0)  // содержимое от края до края
dlg.SetContentPadding(-1) // вернуть умолчание
dlg.ContentPadding()      // действующие отступы: по горизонтали и вертикали
```

Диалог без штатной полосы (`SetChromeless`) по умолчанию рисует содержимое без
отступа: ради этого он и нужен, а рамка фона вокруг боковой панели сводила
затею на нет — закрасить её было нечем, фон диалога один на всё окно. У
обычного диалога умолчание прежнее: `MessageBox` расставляет кнопки и подписи в
расчёте на 14/12.

### Боковая навигация (NavPanel)
...
dlg.SetNavPanel(nav)   // колонка ВО ВСЮ ВЫСОТУ окна, включая полосу заголовка
dlg.SetNavButton(true) // «≡» в полосе сворачивает эту же панель
```

Пункт — это иконка И подпись сразу, и поэтому свёрнутый режим прячет подписи, а
не пункты: от панели остаётся полоска её цвета с иконками, а содержимое
занимает освободившееся место. Пункт без иконки рисуется первой буквой подписи —
иначе свёрнутая панель превращалась бы в полоску пустых строк у того, кто иконки
не завёл.

```go
nav.SetCollapsed(true)   // полоска с иконками
nav.ExpandedWidth = 260  // ширина в двух состояниях
nav.CollapsedWidth = 52
nav.SetSelected(1)       // выбрать программно (OnSelect НЕ зовётся)
nav.ItemRect(i)          // прямоугольник пункта — например, для своей подсказки
```

Колонка занимает левую часть от ВЕРХНЕГО края окна до нижнего: панель рисуется
первой из детей, а подпись заголовка движок докладывает поверх — так цвет
доходит до края, а заголовок остаётся на месте, как в вертикальных вкладках
браузера. Отрисовка детей при этом обрезается по скруглённому корпусу
(`CornerRadius`): панель заливает колонку прямоугольником и иначе срезала бы
углы окна. Пункты при этом начинаются ниже подписи (`TopInset` панель получает от
хоста).

`ContentBounds` отступает вправо от колонки, поэтому виджет из `SetContent`
раскладывается уже по оставшейся области, а свёрнутая панель отдаёт ему
освободившееся место без единой строки в приложении. Начинка полосы заголовка
(`SetTitleBarContent`) тоже начинается ЗА колонкой: поверх панели у неё чужой
фон, и поле выглядело бы наложенным, а не частью полосы. Ширина колонки ограничена
половиной окна: панель, занявшая всё окно, оставила бы содержимое без места и
без объяснений.

Панель — не обязательно `NavPanel`: `SetNavPanel` берёт любой виджет. Ширину у
него спрашивают через `DesiredSize`, а сворачивание — через `SetCollapsed(bool)`,
если он это умеет. То же самое есть у `widget.Window`: `SetNavPanel`,
`NavPanelBounds`. В mac-стиле колонка под полосу не поднимается — там в левом
верхнем углу стоят кнопки окна.

### Минимальный размер окна

```go
win.SetMinSize(800, 600) // ЛОГИЧЕСКИЕ пиксели
win.SetMinSize(0, 0)     // снять ограничение
win.MinSize()            // спросить заданное
```

Логические, а не физические: приложение думает в тех же единицах, что и
виджеты, а пересчёт по HiDPI-масштабу делает окно — и делает заново, когда окно
переезжает на монитор с другим DPI.

Вызывать можно и до `Run()` (применится при создании окна), и во время работы:
окно с двумя режимами — компактным и полным — вправе менять минимум на лету.
Явный вызов отменяет `MinWidth`/`MinHeight` корневого `widget.Window`: явное
конкретнее разметки.

Ограничение соблюдают все три бэкенда: Win32 (`WM_GETMINMAXINFO`), X11
(`WM_NORMAL_HINTS`) и Wayland (`xdg_toplevel.set_min_size`). На macOS — no-op.

### Значок окна

```go
win.SetIcon(icon16, icon32, icon256) // несколько размеров
win.SetIcon()                        // снять значок
```

Размер выбирается ближайший к тому, что просит система: Windows берёт разные
для заголовка (16×16) и для Alt+Tab (32×32), оконные менеджеры X11 — свой под
каждое место. Один размер тоже годится, недостающие система отмасштабирует.

Вызывать можно в любой момент после создания окна — например, чтобы показать
ход длительной операции, — и до `Run()`: значок применится при создании окна.

Работает на **Win32** (`WM_SETICON`) и **X11** (`_NET_WM_ICON`). Бэкенд, который
этого не умеет (Wayland до `xdg-toplevel-icon-v1`), возвращает
`window.ErrIconUnsupported` — молчания здесь нет намеренно: приложение, у
которого значка не видно, должно узнать причину сразу, а не искать её в
настройках рабочего стола.

До `Run()` метод всегда возвращает `nil`: бэкенда ещё нет, и сказать, поддержан
ли значок, не по чему. Значок откладывается и ставится при создании окна — так
же ведёт себя `SetTrayIcon`.
