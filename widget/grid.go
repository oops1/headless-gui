...
func (g *Grid) measureAutoSizes() {
	...
	for _, child := range g.children {
		// Скрытый ребёнок места не занимает.
		//
		// Без этой проверки строка Auto держала высоту виджета, которого на
		// экране нет: «Показать панель инструментов» выключали, а полоса под
		// ней оставалась пустой. WPF ведёт себя так же — Collapsed убирает
		// элемент из раскладки, в отличие от Hidden, который место сохраняет.
		//
		// Строку ФИКСИРОВАННОЙ высоты это не схлопывает, и не должно: её
		// размер задан числом, а не содержимым. Чтобы полоса исчезала вместе
		// с содержимым, строке нужен размер Auto — так же, как в WPF.
