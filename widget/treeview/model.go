...
type TreeViewItem struct {
	...
	// Foreground — свой цвет текста узла.
	//
	// Нулевая альфа означает «цвет темы»: узлы, которым цвет не задавали,
	// рисуются как прежде. Нужно там, где часть строки второстепенна —
	// имя ветки рядом с именем репозитория.
	Foreground color.RGBA

	// Bold — рисовать текст узла жирным.
	//
	// Начертание, а не «стиль» с набором флагов: курсив в списке репозиториев
	// не нужен никому, а заводить перечисление ради одного значения — значит
	// придумывать за пользователя.
	Bold bool
