...
func checkNoStaleContent(t *testing.T, eng *engine.Engine, what string) {
	...
	// Шум анимаций: несколько полных кадров подряд. Одной пары мало —
	// свечение едет, и за два кадра оно проходит лишь часть своего пути;
	// область шума собирается по всем парам, чтобы накрыть весь ход.
	...
	// Область, где что-то анимируется, исключается ЦЕЛИКОМ, а не по точкам:
	// свечение едет непрерывно, и точка, совпавшая в двух кадрах подряд, в
	// третьем всё равно другая. Внутри этой области сравнивать нечем; всё
	// остальное — а это почти весь экран, и там как раз кнопки и меню, на
